...
def noise_score(lat: float, lng: float, radius_m: int = 500,
    ...
    if _TRANSFER_ENABLED:
        try:
            ...
            # NOTE this cancels out of the final road term by arithmetic --
            # road_leq_t = road_leq + (t_road_lden - rd_only_lden) and
            # rd_only_lden = road_leq + const -- so the physics road level, and
            # every barrier-screening dB inside it, does NOT reach the score
            # whenever the transfer path is active. Measured 2026-07-25: zeroing
            # all 15 dB of building screening moves the score by 0 dB on 5 of 8
            # test points. Known and deliberate; see the SSOT before "fixing" it.
            rd_only_lden = _lden(
