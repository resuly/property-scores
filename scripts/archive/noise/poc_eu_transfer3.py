...
def main():
    ...
    KEYS = fkeys()
    cache = "data/eu/transfer3_cache.npz"
    if os.path.exists(cache):
        ...
        for i, r in enumerate(nlp):
            f = feats(con, "nlr", "nlb", "poi", float(r["lat"]), float(r["lng"]))
    ...
    rf.fit(Xnl, ynl)
    raw = rf.predict(Xau)
    a = st(raw, yau); print(f"\nRAW NL->AU (road+bldg+DEM+LC+POI): MAE=%.1f bias=%+.1f r=%.2f std=%.1f (tgt %.1f)" % (a[0], a[1], a[2], a[3], yau.std()), flush=True)
