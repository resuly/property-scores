...
def main():
    ...
    con.execute("CREATE TABLE ukr AS SELECT class,geometry,xmin,ymin FROM read_parquet('data/uk/uk_roads.parquet')")
    con.execute("CREATE TABLE ukb AS SELECT h,clng,clat FROM read_parquet('data/uk/uk_buildings.parquet') ORDER BY clng,clat")
    print(f"  loaded ({time.time()-t0:.0f}s)", flush=True)

    KEYS = fkeys()
    cache = "data/eu/transfer4_cache.npz"
    if os.path.exists(cache):
        d = np.load(cache, allow_pickle=True); Xnl, ynl, Xau, yau, cau = d["Xnl"], d["ynl"], d["Xau"], d["yau"], list(d["cau"])
    else:
        train = [("nlr","nlb",r) for r in csv.DictReader(open("data/eu/nl_train_points.csv"))]
        train += [("ukr","ukb",r) for r in csv.DictReader(open("data/uk/uk_train_points.csv"))]
        Xnl, ynl = [], []
        for i,(rt,bt,r) in enumerate(train):
            f = feats(con, rt, bt, "poi", float(r["lat"]), float(r["lng"]))
            Xnl.append([f[k] for k in KEYS]); ynl.append(float(r["lden"]))
            if (i + 1) % 1500 == 0: print(f"  EU(NL+UK) {i+1}/{len(train)} ({time.time()-t0:.0f}s)", flush=True)
        Xau, yau, cau = [], [], []
